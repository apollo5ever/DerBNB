...
70 LET renter = LOAD(property_id_booker_booking_id) // dero address of the renter that was stored in ConfirmBooking function
80 LET deposit = LOAD(property_id_damage_deposit) // dero amount stored in SetDamageDepositAmount function
90 IF damage_amount_in_dero > deposit THEN GOTO 1000 // damage amount can't be more than deposit amount
100 IF damage_amount_in_dero = "" LET damage = 0 ELSE LET damage = damage_amount_in_dero // default to 0 if damage_amount_in_dero is left blank
